...
#         return box_similarities.view(B, -1, self.num_classes)
        ...
        '''
        For each proposal box > obj thres, extract features (extract_backbone_features) and calculate similarity
        (get_cosim_mini_batch)
        '''
        ...
        for b in range(B):
            valid = boxes[b, :, 4] > obj_thres
            cur_boxes = boxes[b, valid, :4].long()

            if len(cur_boxes) == 0:
                all_box_sims.append(torch.empty(0, self.num_classes, device=images.device))
                continue

            crops = torch.stack([
                images[b, :, y1:y2, x1:x2]
                for x1, y1, x2, y2 in cur_boxes
            ])

            feats = extract_features(crops, self.backbone, self.backbone_type, text=self.text)
            sims = self.get_cosim_mini_batch(feats, self.embedding, normalize=normalize)

            # global aggregation per box
            box_sims = sims.mean(dim=[2, 3])
            all_box_sims.append(box_sims)

        return torch.cat(all_box_sims, dim=0)
