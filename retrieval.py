...
def get_model(args):
    if args.backbone == 'openclip':
        ...
        def preprocess_train(images):
            return processor(images=images, return_tensors="pt")["pixel_values"].squeeze(1)
    ...
    for batch_images, batch_texts, inds in tqdm(dataloader, total=n_batches):
        ...
        with torch.no_grad():
            if args.backbone == 'openclip':
                ...
                batch_image_features = model.get_image_features(batch_images)
                batch_text_features = model.get_text_features(batch_texts)
